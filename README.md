...
- **값객체**를 사용하여 객체의 조건을 보장하는 역할을 위임 
- 상태와 로직을 따로 관리할 수 있는 **일급 컬렉션**의 사용 
- 테스트하기 어려운 값(ex.Random)에 대한 의존을 역전시켜 특정 구현에 의존하지 않고 **테스트하기 쉬운 코드**로 변경 
- 매직 넘버에서 **의미있는 상수**로 변환하기
